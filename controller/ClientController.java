...
public class ClientController {
	...
	private Point playOpening(int progress, byte color) {
		...
		for (int variant = 0; variant < openings.length; variant++) { // voor elke opening sequence
			...
			System.out.println("openingB:");
			for (int i = 0; i < 8; i++){
				for (int j = 0; j < 8; j++){
					System.out.print(openingB[i][j]);
				}
				System.out.println("");
			}
			if(Arrays.deepEquals(mBoard, openingB)){
				System.out.println("--------------------------------------------------EEN MATCH-----------------------------------------------------------------");
				System.out.println(openings[variant].length);
				if(openings[variant].length>variantStep+1)
					if(color == currentGame.WHITE){
						System.out.println(openings[variant][variantStep]+"BL"+openings[variant][variantStep+1]);
						return new Point(openings[variant][(variantStep)+1]-49, openings[variant][(variantStep)]-97);
					}
					else{
						System.out.println(openings[variant][variantStep]+"WH"+openings[variant][variantStep+1]);
						return new Point(openings[variant][(variantStep)+1]-49, openings[variant][(variantStep)]-97);
					}
			}
		}
